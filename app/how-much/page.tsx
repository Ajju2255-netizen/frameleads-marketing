...
const URL = "https://frameleads.com/how-much";

export const metadata: Metadata = {
	title: "How-much Pricing Guides — Cost Bands for Marketing Decisions | Frameleads",
	description:
		"How-much pricing guides — cost bands, factors that move price, and India-specific rate cards for marketing services + tools + tactics.",
	alternates: { canonical: URL },
};

const items = questions.filter((q) => q.kind === "how-much");

const schema = [
	{
		"@context": "https://schema.org",
		"@type": "CollectionPage",
		name: "Frameleads How-much Pricing Guides",
		url: URL,
		hasPart: items.map((q) => ({
			"@type": "Article",
			name: q.title,
...
export default function HowMuchHubPage() {
	return (
		<>
			...
			<main>
				<header className="relative overflow-hidden border-b border-[#FFE4D6] bg-gradient-to-b from-[#FFF7F2] via-white to-white">
					...
					<div className="mx-auto max-w-3xl px-6 pt-24 pb-12 sm:pt-28 sm:pb-16">
						<Breadcrumb items={[{ href: "/", label: "Home" }, { label: "How-much" }]} />
						...
						<h1 className="mt-4 font-poppins text-[34px] sm:text-[44px] font-bold leading-[1.05] tracking-tight text-[#2D3748]">
							How-much pricing guides
						</h1>
						<p className="direct-answer mt-4 max-w-2xl text-[16px] sm:text-[17px] leading-relaxed text-[#5A5A5A]">
							Cost bands, factors that move price, and India-specific rate cards for marketing services, tools, and tactics.
						</p>
				...
				<section className="mx-auto max-w-3xl px-6 py-12 sm:py-16">
					<ul className="grid grid-cols-1 gap-3 sm:grid-cols-2">
						{items.map((q) => (
							<li key={q.slug}>
								<Link href={`/how-much/${q.slug}`} className="group block h-full rounded-2xl border border-[#FFE4D6] bg-white p-5 shadow-[0_2px_10px_rgba(45,55,72,0.04)] transition-all duration-300 hover:-translate-y-0.5 hover:border-[#FF6B35]/40 hover:bg-[#FFF7F2]/60 hover:shadow-md">
				...
				<CTABlock
					variant="audit"
					headline="Want exact pricing for your situation?"
					body="A 30-min audit gives an exact band against your specific stage, ICP, and unit economics."
